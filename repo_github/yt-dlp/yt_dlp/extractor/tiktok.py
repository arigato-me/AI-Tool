...
class DouyinIE(TikTokBaseIE):
    ...
    @staticmethod
    def _build_detail_query(aweme_id):
        return {
    ...
    def _apply_auto_cookies(self, force=False):
        cookies = self._get_cookies(self._WEBPAGE_HOST)
        if not force and cookies.get('ttwid') and cookies.get('s_v_web_id'):
            return False

        from ..utils.douyin_cookies import fetch_douyin_cookies

        if force:
            self.to_screen('Refreshing Douyin cookies')
        else:
            self.to_screen('Generating Douyin cookies')
        expire_time = int(time.time()) + 86400 * 30
        for name, value in fetch_douyin_cookies(self._WEB_UA).items():
            for domain in ('.douyin.com', 'www.douyin.com'):
                self._set_cookie(domain, name, value, expire_time=expire_time)
        return True

    def _fetch_detail(self, video_id):
        query = self._build_detail_query(video_id)
        try:
            from ..utils.abogus import ABogus
            query['a_bogus'] = ABogus().get_value(query)
        except ImportError as e:
            raise ExtractorError(
                'Douyin extraction requires the gmssl package. '
                'Install with: pip install gmssl', expected=True) from e

        return traverse_obj(self._download_json(
            self._DETAIL_API, video_id,
            'Downloading web detail JSON', 'Failed to download web detail JSON',
            query=query, fatal=False, headers={
                'User-Agent': self._WEB_UA,
                'Referer': f'{self._WEBPAGE_HOST}video/{video_id}',
            }), ('aweme_detail', {dict}))

    def _real_extract(self, url):
        video_id = self._match_id(url)
        self._apply_auto_cookies()
        detail = self._fetch_detail(video_id)
        if not detail:
            self._apply_auto_cookies(force=True)
            detail = self._fetch_detail(video_id)
        if not detail:
            raise ExtractorError(
                'Unable to download video details. '
                'Try again or provide fresh cookies with --cookies',
                expected=True)

        return self._parse_aweme_video_app(detail)


class DouyinVMIE(InfoExtractor):
    _VALID_URL = r'https?://v\.douyin\.com/(?P<id>\w+)'
    IE_NAME = 'v.douyin'

    _TESTS = [{
        'url': 'https://v.douyin.com/IZ90MChFahQ/',
        'only_matching': True,
    }]

    def _real_extract(self, url):
        # Douyin's final video page doesn't support HEAD (404s), unlike TikTokVMIE's target - need a real GET
        new_url = self._request_webpage(
            url, self._match_id(url), headers={'User-Agent': 'facebookexternalhit/1.1'}).url
