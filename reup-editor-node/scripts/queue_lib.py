...
SERVICE = "editor"  # đổi giá trị này ở mỗi node khi copy sang
...
def pop_job(conn: redis.Redis, timeout: int = 5) -> str | None:
    ...
    Bug thật gặp lần 2 (test cắt mạng thật — chính node này, xem ghi chú fix heartbeat cùng
    đợt): mất kết nối Redis hẳn (network blip, DNS resolution fail...) ném
    `redis.exceptions.ConnectionError` — KHÔNG phải subclass của `TimeoutError`, lọt qua except
    cũ, crash cả worker. Bắt rộng ra `redis.exceptions.RedisError` (class cha chung của mọi lỗi
    redis-py, gồm cả 2 loại trên) — cùng coi như 1 vòng poll rỗng, để lần gọi kế tự thử kết nối
    lại, không cần tự viết logic reconnect."""
    try:
